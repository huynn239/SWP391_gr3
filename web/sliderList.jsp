...
<head>
    ...
    <style>
        ...
        .search-container label, .sort-container label, .filter-container label {
            font-weight: 600;
            margin-right: 10px;
            color: #495057;
        }
        .search-container input, .sort-container select, .filter-container select {
            border-radius: 5px;
    ...
    <div class="control-panel">
        <div class="row">
            ...
            <div class="col-md-4 sort-container">
                ...
                <select id="sortOrder" onchange="location.href = 'sliderList?sortBy=created_at&sortOrder=' + this.value + '&page=${currentPage}&keyword=${keyword}&statusFilter=${statusFilter}'" class="form-control d-inline-block w-auto">
                    <option value="DESC" ${sortOrder == 'DESC' ? 'selected' : ''}>Newest</option>
                    <option value="ASC" ${sortOrder == 'ASC' ? 'selected' : ''}>Latest</option>
                </select>
            </div>
            <div class="col-md-4 filter-container text-right">
                <label for="statusFilter">Filter Status:</label>
                <select id="statusFilter" onchange="location.href = 'sliderList?statusFilter=' + this.value + '&page=${currentPage}&keyword=${keyword}&sortBy=${sortBy}&sortOrder=${sortOrder}'" class="form-control d-inline-block w-auto">
                    <option value="" ${empty statusFilter ? 'selected' : ''}>All</option>
                    <option value="true" ${statusFilter == 'true' ? 'selected' : ''}>Actived</option>
                    <option value="false" ${statusFilter == 'false' ? 'selected' : ''}>Inactive</option>
                </select>
                <a href="sliderList?action=addSlider" class="btn btn-add ml-2"><i class="fa fa-plus"></i> Add new Slider</a>
            </div>
        </div>
    </div>
        ...
        <c:when test="${not empty sliders}">
            ...
            <div class="pagination">
                <c:if test="${currentPage > 1}">
                    <a href="sliderList?page=${currentPage - 1}&sortBy=${sortBy}&sortOrder=${sortOrder}&keyword=${keyword}&statusFilter=${statusFilter}" class="btn btn-primary" style="margin: 0; color: black; border: 1px solid; border-radius: 20px; background: white;">← Previous</a>
                </c:if>

                <c:forEach var="i" begin="1" end="${totalPages}">
                    <a href="sliderList?page=${i}&sortBy=${sortBy}&sortOrder=${sortOrder}&keyword=${keyword}&statusFilter=${statusFilter}" class="btn ${currentPage == i ? 'btn-dark' : 'btn-outline-primary'}">${i}</a>
                </c:forEach>

                <c:if test="${currentPage < totalPages}">
                    <a href="sliderList?page=${currentPage + 1}&sortBy=${sortBy}&sortOrder=${sortOrder}&keyword=${keyword}&statusFilter=${statusFilter}" class="btn btn-primary" style="margin:0; color: black; border: 1px solid; border-radius: 20px; background: white;">Next →</a>
                </c:if>
            </div>
        </c:when>
        <c:otherwise>
            <div class="alert alert-warning text-center">Không có slider nào phù hợp với tiêu chí.</div>
        </c:otherwise>
    </c:choose>
</div>
